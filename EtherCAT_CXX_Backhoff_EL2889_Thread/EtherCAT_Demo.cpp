...
static void EtherCAT_Exit(int a){EtherCAT_Running_Flag = false;}
...
static void * EtherCAT_Thread(void *)
{
    ...
    printf("EtherCAT Master Stopped\n");
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    pthread_setschedparam(thread1, SCHED_FIFO, &param1); // pthread_setschedparam函数针对线程优先级进行设置，SCHED_RR表示将根据优先级使用轮转的实时调度策略，允许同优先级的线程进行轮转运行

    // 设置线程1的CPU亲和性（绑定到CPU 0）
    CPU_ZERO(&cpuset);
    CPU_SET(15, &cpuset); // 设置线程1只能在CPU 0上运行
