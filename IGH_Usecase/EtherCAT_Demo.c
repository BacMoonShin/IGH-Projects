...
static void check_domain_state(ec_domain_t *domain, ec_domain_state_t *domain_state)
{
    ...
    {
        printf("Domain: Working_Counter %u.\n", ds.working_counter);
    }
...
static void check_master_state(ec_master_t *master, ec_master_state_t *master_state)
{
    ...
    {
        printf("Now %u slave(s) is(are) responding.\n", ms.slaves_responding);
    }
...
static void init_EtherCAT_master(struct OTModule *module)
{
    ...
    // 填充相关PDOS信息，表示一个PDO对象中所包含的PDO_entry信息，即PDO对象中含有的所有映射信息，包含映射目标对象字典索引、子索引、bit大小
    ...
    // 构建每一个PDO的信息，PDO信息包含该PDO的对象字典索引，该PDO对象中保存的PDO映射个数以及ec_pdo_entry_info_t列表指针
    // ec_pdo_info_t 用于初始化RxPDO以及TxPDO的映射表信息，结构为{RxPDO或TxPDO的对象字典索引，PDO_entry的个数，ec_pdo_entry_info_t中RxPDO或TxPDO映射信息对应起始位}
    ...
    // 构建同步管理器的运行信息，包括该同步管理器的编号、该同步管理器的数据流方向、该同步管理器管理的PDO数量、该同步管理器管理的PDO信息ec_pdo_info_t列表、看门狗工作模式
    // ec_sync_info_t {同步管理器编号，工作模式，PDO数量，RxPDO或TxPDO映射表索引位置，看门狗模式}
...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        if(!(el2889.BackHoff_EL2889_State.operational)){
          printf("*Waiting EL2889 state to become operational*\n");
        }
        else{
