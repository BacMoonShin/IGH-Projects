...
static ELMO_SDO_Data SDO_Data = {1, 1, 1, 1, 1, 1, 1, 1, 0, CSP}; // TODO: 该处的SDO参数配置需要根据实际情况修改
static std::array<ELMO *, 14> Arm_Elmos;
...
static void Elmo_Display_Function(ELMO& _Elmo, int& _Position)
{
    ...
    {
        ETHERCAT_INFO("Elmo%d: act position = %d, act torque = %d, status = 0x%x, drive_state = %d, drive_mode = %d",
                      _Position, _Elmo.domain_data.TxPDO.actual_position, _Elmo.domain_data.TxPDO.torque_input, _Elmo.domain_data.TxPDO.status_word, _Elmo.data.drive_state, _Elmo.data.drive_mode);
    }
}
...
void EtherCAT_Start()
{
    ...
    for(auto &elmo : Arm_Elmos)
    {
        elmo = new ELMO(Master, 0, position, SDO_Data);
        ++position;
    }
